  ...
  return (
    <div className="w-[210mm] min-h-[297mm] mx-auto bg-white text-gray-800 shadow-xl print:shadow-none flex flex-col font-sans" style={{ fontFamily: 'ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif' }}>
      ...
      <header className="p-10 text-white text-center" style={{ backgroundColor: theme.primaryColor }}>
        ...
        <div className="flex flex-wrap justify-center gap-6 mt-6 text-sm opacity-80">
          {personal.email && <div className="flex items-center gap-2"><Mail size={16}/> {personal.email}</div>}
          {personal.phone && <div className="flex items-center gap-2"><Phone size={16}/> {personal.phone}</div>}
          {personal.location && <div className="flex items-center gap-2"><MapPin size={16}/> {personal.location}</div>}
        </div>
      </header>
        ...
        <div className="grid grid-cols-2 gap-8 mt-auto">
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-2">
              {skills.languages.concat(skills.frameworks).concat(skills.tools).map(skill => (
                <span key={skill} className="px-3 py-1 bg-gray-100 text-gray-700 text-sm font-medium rounded-md border border-gray-200">
                  {skill}
                </span>
              ))}
            </div>
